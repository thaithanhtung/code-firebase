...
export const initAuth = roles => (dispatch, getState) => {
  ...
  const setUser = userState => {
    ...
    if (!roles.find(role => role === userRole)) {
      if (!(state.routing.location.pathname === '/dashboard/beta')) {
        dispatch(push('/dashboard/beta'));
      }
...
export function login(username, password, dispatch) {
  ...
  notification.open({
    type: 'success',
    message: `${username} đã đăng nhập thành công`,
    description:
      'Chào mừng bạn đã đăng nhập vào web thành công nhé',
  });
  return true;
